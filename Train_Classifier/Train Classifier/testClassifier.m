...
while toc < runtime 
    % Compute the frame rate averaged over the last 10 frames
        ...
        fps = 1/(toc-timeTracker);
        timeTracker = toc;
        % Get a new frame from the camera
        img = snapshot(vid);
        
        %    bboxfull = step(options.detectorfull, img);
        bboxpist = step(options.detectorpist, img);
        bboxFace = step(options.detectorFace, img);
        average1 = 0; average2 = 0;
        %
        detectedImg = insertShape(img, 'rectangle', bboxpist, 'Color', [255 0 0]);
        detectedImg = insertShape(detectedImg, 'rectangle', bboxFace, 'Color', [0 0 255]);
        
        % Display image
        %    Note: use imagesc() instead of imshow() (it's faster).
        figure(1);imagesc(detectedImg); axis off
        title(['FPS: ' sprintf('%2.1f:%2.1f:%2.1f', average1,average2,fps)]); drawnow
